...
function swap_arc!(diagram::Diagram)
    ...
    if r<rand()
        ...
        diagram.total_dispersion-=dispersion(line_box[line_index], m, μ)
...
function scale!(diagram::Diagram, order::Int64,m::Int64,μ::Float64,ω::Int64, samples::Int64)
    ...
    #    total_dis+=arc_dispersion(arc,ω)
    #end

    #for arc in end_arc_box
        #period=arc.period
        #total_dis+=-ω*(τ+period[2]-period[1])
    #    total_dis+=end_arc_dispersion(arc ,ω, τ)
    #end

    #for i in 1:line_box_length
    #    total_dis+=dispersion(line_box[i], m, μ)
    #    if i < arc_box_length && i < end_arc_box_length
    #        total_dis+=arc_dispersion(arc_box[i],ω)
    #        total_dis+=end_arc_dispersion(end_arc_box[i] ,ω, τ)
    #    elseif i < arc_box_length
    #        total_dis+=arc_dispersion(arc_box[i],ω)
    #    elseif i < end_arc_box_length
    #        total_dis+=end_arc_dispersion(end_arc_box[i],ω, τ)
    #    else
    #    end
    #end
    ...
    total_dis+=diagram.total_dispersion
    
    #total_dis+= sum(map!(p->dispersion(p,m, μ), line_box))
    #total_dis+= sum(map(p->-ω*(p.period[2]-p.period[1]), arc_box))
    #total_dis+= sum(map(p->-ω*(τ+p.period[2]-p.period[1]), end_arc_box))
    if total_dis == 0.0
        total_dis = dispersion(line_box[1], m, μ)
    end
